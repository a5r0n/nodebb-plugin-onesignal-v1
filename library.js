...
onesignal.init = function (data, callback) {
  ...
  // Admin setup routes
  data.router.get(
    "/admin/plugins/onesignal",
    data.middleware.admin.buildHeader,
    pluginControllers.renderACP
  );
  ...
  data.router.get(
    "/onesignal/settings",
    pluginMiddleware.hasConfig,
    pluginMiddleware.setupRequired,
    data.middleware.buildHeader,
    pluginControllers.renderSettings
  );
  data.router.get(
    "/api/onesignal/settings",
    pluginMiddleware.setupRequired,
    pluginControllers.renderSettings
  );
  data.router.get(
    "/api/me/onesignal/devices",
    pluginMiddleware.isLoggedIn,
    pluginControllers.getPlayerIds
  );
  data.router.post(
    "/api/me/onesignal/devices",
    pluginMiddleware.isLoggedIn,
    pluginMiddleware.addDevice,
    pluginControllers.getPlayerIds
  );
  ...
  data.router.get(
    ...
    pluginMiddleware.OneSignalSDKWorker
  );
  data.router.get(
    "/OneSignalSDKUpdaterWorker.js",
    pluginMiddleware.OneSignalSDKUpdaterWorker
  );
  ...
  db.getObject("settings:onesignal", function (err, config) {
    if (!err && config) {
      ...
      winston.info(
        "[plugins/onesignal] Please complete setup at `/admin/onesignal`"
      );
    }
  });
  ...
  SocketPlugins.onesignal = {
    settings: {
      ...
      app_id: onesignal.settings.app_id,
    },
...
onesignal.redirectSetup = function (req, res) {
  ...
  if (process.env.NODE_ENV === "development") {
    winston.info(
      "[plugins/onesignal] New association, redirecting user to: " +
  ...
  async.parallel(
    {
      ...
      settings: async.apply(db.getObjectsFields, settingsKeys, [
        "onesignal:enabled",
        "topicPostSort",
        "language",
      ]),
    ...
    function (err, results) {
      ...
      if (results.hasOwnProperty("players")) {
        uids.forEach(function (uid, index) {
          ...
          if (
            results.settings[index]["onesignal:enabled"] === null ||
            parseInt(results.settings[index]["onesignal:enabled"], 10) === 1
          ) {
            pushToUid(
              uid,
              notifObj,
...
function pushToUid(uid, notifObj, players, settings, app_id) {
  ...
  async.waterfall([
    function (next) {
      ...
      notifObj.bodyLong = escapeHtml(
        striptags(unescapeHtml(notifObj.bodyLong || ""))
      );
      async.parallel(
        {
    ...
    function (data, next) {
      var payload = {
        ...
        url:
          notifObj.path ||
          nconf.get("url") + "/topic/" + data.topicSlug + "/" + data.postIndex,
      };

      winston.verbose(
        "[plugins/onesignal] Sending push notification to uid " +
          uid +
          "players: " +
          players
      );
      request.post(
        ...
        function (err, request, result) {
          if (err) {
            winston.error(
              "[plugins/onesignal (uid: " + uid + ")] " + err.message
            );
          } else if (result) {
            try {
  ...
  async.waterfall(
    [
      ...
      function (playersData, next) {
        ...
        db.setObjectField(
          "users:onesignal:players",
          uid,
          JSON.stringify(Array.from(players)),
          next
        );
      },
    ],
...
onesignal.settings.load = function (socket, data, callback) {
  if (socket.hasOwnProperty("uid") && socket.uid > 0) {
    db.getObjectFields(
      "user:" + socket.uid + ":settings",
      ["onesignal:enabled", "onesignal:target"],
      callback
    );
  } else {
    callback(new Error("not-logged-in"));
...
onesignal.settings.app_id = function config(socket, data, callback) {
  if (socket.hasOwnProperty("uid") && socket.uid > 0) {
